...
class NotificationsRepository
    ...
    constructor(
        ...
        override fun readNotifications() =
            rxSingle { notificationsApi.readNotifications() }
                .retryWhen(userTokenRefresher)
                .map { emptyList<Notification>() }

        override fun getNotifications(page: Int) =
            rxSingle { notificationsApi.getNotifications(page) }
                .retryWhen(userTokenRefresher)
                .compose(ErrorHandlerTransformer())
                .map { it.map { response -> NotificationMapper.map(response) } }

        override fun getNotificationCount() =
            rxSingle { notificationsApi.getNotificationCount() }
                .retryWhen(userTokenRefresher)
                .compose(ErrorHandlerTransformer())

        override fun readHashTagNotifications() =
            rxSingle { notificationsApi.readHashTagsNotifications() }
                .retryWhen(userTokenRefresher)
                .compose(ErrorHandlerTransformer())
                .map { it.map { response -> NotificationMapper.map(response) } }

        override fun getHashTagNotifications(page: Int) =
            rxSingle { notificationsApi.getHashTagsNotifications(page) }
                .retryWhen(userTokenRefresher)
                .compose(ErrorHandlerTransformer())
                .map { it.map { response -> NotificationMapper.map(response) } }

        override fun getHashTagNotificationCount() =
            rxSingle { notificationsApi.getHashTagsNotificationsCount() }
                .retryWhen(userTokenRefresher)
                .compose(ErrorHandlerTransformer())
    }
