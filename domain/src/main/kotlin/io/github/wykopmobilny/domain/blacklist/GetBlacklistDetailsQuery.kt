    ...
    constructor(
        ...
        override fun invoke() =
            combine(
                ...
                val content =
                    if (blacklist == null || blacklist.users.isEmpty() && blacklist.tags.isEmpty()) {
                                        ...
                                        blacklist.users
                                            ...
                                            .map { user ->
                                                BlacklistedElementUi(
                                                    name = user,
                                                    state =
                                                        when (val state = viewStateUi.usersState[user]) {
                                                            is ItemState.Error -> {
                                        ...
                                        blacklist.tags
                                            ...
                                            .map { tag ->
                                                BlacklistedElementUi(
                                                    name = tag,
                                                    state =
                                                        when (val state = viewStateUi.tagsState[tag]) {
                                                            is ItemState.Error -> {
                                                                BlacklistedElementUi.StateUi.Error(
                                                                    showError = {
                                                                        viewState.update {
                ...
                BlacklistedDetailsUi(
                    errorDialog =
                        viewStateUi.generalResource.failedAction?.let { error ->
                            ErrorDialogUi(
                                error = error.cause,
