...
class LinkInteractor @Inject constructor(val linksApi: LinksApi) {
    ...
    fun digLink(link: Link): Single<Link> = linksApi.voteUp(link.id)
        .map {
    ...
    fun buryLink(link: Link, reason: Int): Single<Link> = linksApi.voteDown(link.id, reason)
        .map {
            link.voteCount = it.diggs
            link.buryCount = it.buries
            link.userVote = "bury"
    ...
    fun removeVote(link: Link): Single<Link> = linksApi.voteRemove(link.id)
        .map {
            link.voteCount = it.diggs
            link.buryCount = it.buries
            link.userVote = null
    ...
    fun markFavorite(link: Link): Single<Link> = linksApi.markFavorite(link.id)
        .map {
